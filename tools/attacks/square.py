            ...
            if self.norm == 'Linf':
                ...
                for i_iter in range(self.n_queries):
                    ...
                    if len(idx_to_fool) == 0:
                        print(f"Iter: {i_iter}, target: {y}, predicted: {torch.argmax(self.get_logits(x_best))}")
                        try:
                            self.data_sum[y.cpu().item()].append(i_iter)
                        except:
                            self.data_sum = []
                        break #################################
                    ...
                    if ind_succ.numel() == n_ex_total:
                        ...
                        try:
                            ...
                            self.data_sum = []
                        break
                    if i_iter == self.n_queries - 1:
                        print(f"Attack failed: Iter: {i_iter}, target: {y}, predicted: {torch.argmax(self.get_logits(x_best))}")
