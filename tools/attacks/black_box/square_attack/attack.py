...
def pseudo_gaussian_pert_rectangles(x, y):
    #  fills the array delta with values that correspond to a "pseudo-Gaussian" distribution
    delta = np.zeros([x, y])
    x_c, y_c = x // 2 + 1, y // 2 + 1
...
def loss_custom(y, logits, targeted=False, loss_type='margin_loss'):
    ...
    if loss_type == 'margin_loss':
        ...
        #print("3333333333333", y.astype(int))
        probs = softmax(logits)
        #print(probs, probs.shape)
        #loss = -np.log(probs[np.array(y, dtype=int)])
         # Add a small epsilon value to avoid logarithm of zero
        epsilon = 1e-8
...
def predict_custom(model, x):
    ...
    with torch.no_grad():  # otherwise consumes too much memory and leads to a slowdown
        for i in range(n_batches):
            ...
            logits = model(x_batch_torch).cpu().numpy()
...
def square_attack_l2(model, x, y, corr_classified, eps, n_iters, p_init, metrics_path, targeted, loss_type):
    ...
    #x, y = x[corr_classified], y[corr_classified] ### just take correct predicted 

    ### initialization
    #print("------------------", x.shape)
    delta_init = np.zeros(x.shape) ### n x c x h x w
    s = h // 5 ### the size of the square
    ...
    for counter in range(h // s):
        ...
        for counter2 in range(w // s):
            delta_init[:, :, center_h:center_h + s, center_w:center_w + s] += meta_pseudo_gaussian_pert(s).reshape(
                [1, 1, s, s]) * np.random.choice([-1, 1], size=[x.shape[0], c, 1, 1]) ### n_subsquares = 2; n x c x s x s
            center_w += s
        center_h += s
    #####
    ...
    logits = predict_custom(model, x)
    #print("222222222222222222", logits.shape, y.shape)
    #print("222222222222222222", type(logits), type(y))
    loss_min = loss_custom(y, logits, targeted, loss_type=loss_type)
    margin_min = loss_custom(y, logits, targeted, loss_type='margin_loss')
    ##################################################################################
    
    n_queries = np.ones(x.shape[0])  # n  ### ones because we have already used 1 query

    time_start = time.time()
    s_init = int(np.sqrt(p_init * n_features / c)) ### p_init * h * w
    ...
    for i_iter in range(n_iters):
        ...
        #print("xxxxxxxxxxxxxxxxxxxxxxxx", type(delta_curr))
        ...
        logits = predict_custom(model, x_new)
        loss = loss_custom(y_curr, logits, targeted, loss_type=loss_type)
        margin = loss_custom(y_curr, logits, targeted, loss_type='margin_loss')
