...
class Model:
    ...
    def loss(self, y, logits, targeted=False, loss_type='margin_loss'):
        ...
        if loss_type == 'margin_loss':
            preds_correct_class = (logits[:,:10] * y).sum(1, keepdims=True)
            diff = preds_correct_class - logits[:,:10]  # difference between the correct class and all other classes
...
class ModelTF(Model):
    ...
    def predict(self, x):
        ...
        #return logits
        #print("3333333333333333333333333333", len(logits_list))
        if logits_list: 
            return np.vstack(logits_list)
        else:
            return None  # or return an error message or value
