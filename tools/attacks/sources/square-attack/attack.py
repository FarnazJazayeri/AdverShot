...
def square_attack_l2(model, x, y, corr_classified, eps, n_iters, p_init, metrics_path, targeted, loss_type):
    ...
    np.random.seed(0)
    #print("----------------------------------------", x.shape)
    #print("----------------------------------------", corr_classified, corr_classified.shape)
    min_val, max_val = 0, 1
    c, h, w = x.shape[1:]
    n_features = c * h * w
    ...
    #print("33333333333333333333333333333333333", x_best.shape)
    ...
    print('Maximal norm of the perturbations: {:.5f}'.format(np.amax(curr_norms_image)))
    print("-------------------------Result:-----------------------------")
    print(n_queries.shape, type(n_queries))
    print(x_best.shape, type(x_best))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack', type=str, default='square_l2', choices=['square_linf', 'square_l2'], help='Attack.')
    ...
    parser.add_argument('--n_ex', type=int, default=1000, help='Number of test ex to test on.')
    ...
    print("111111111111111111111111111111 ----------- x_test, y_test")
    print(x_test.shape, type(x_test))
    print(y_test.shape, type(y_test), y_test.min(), y_test.max())
    ...
    corr_classified = logits_clean[:,:10].argmax(1) == y_test
    num_true = np.count_nonzero(corr_classified)
    num_false = np.count_nonzero(~corr_classified)
    print(f"Num True: {num_true}, Num False: {num_false}")
    print("222222222222222222222222222222222222222222 -------------- logits_clean, corr_cls")
    print(logits_clean.shape, type(logits_clean))
    print(corr_classified.shape, type(corr_classified ))
