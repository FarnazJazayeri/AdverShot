...
def train(model, train_loader, val_loader, optimizer, criterion, device, epoch=100, weight_dir="weights/checkpoints", store_dir="weights/checkpoints", num_classes=22, pretrain=False, attack=None, data_attack="all", beta=1.0, lam_sep=1.0, lam_rec=1.0, eps_update=0, eps_delta=8/255, adv_threshold=0.5):
    ...
    for epoch in range(epoch):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            loss_robust = (1.0 / batch_size) * criterion_kl(F.log_softmax(adv_outputs, dim=1),
                                                            F.softmax(model(images.float()), dim=1))
            
            if data_attack == "1vs1":
                cls_loss = criterion(output, labels)
                loss = adv_cls_loss + cls_loss + sep_loss + rec_loss + loss_robust
            else:
