...
def train(model, train_loader, val_loader, optimizer, criterion, device, epoch=100, weight_dir="weights/checkpoints", store_dir="weights/checkpoints", num_classes=22, pretrain=False, attack=None, data_attack="all",
 beta=1.0, eps_update=0, eps_delta=8/255, adv_threshold=0.5):
    ...
    for epoch in range(epoch):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            logits_adv = model(images_adv.float())
            if eps_update > 0:
                eps = attack.eps
                res = predict(logits, labels)
                res_adv = predict(logits_adv, labels)
            ...
            if data_attack == "1vs1":
                ...
                loss = loss_adv + float(beta) * loss_robust + loss_cls
            else:
                loss = loss_adv + float(beta) * loss_robust
            ################################################
            loss.backward()
