...
class Meta(nn.Module):
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry, need_adv=True):
        ...
        for i in range(task_num):
            ...
            fast_weights = list(map(lambda p: p[1] - self.update_lr * p[0], zip(grad, self.net.parameters())))  ########## fast_weights are updated from self.net.parameters()  #################
            ...
            if need_adv:
                ...
                label = y_qry[i]
                optimizer.zero_grad()
                adv_inp_adv = at.attack(self.net, fast_weights, data, label)  ############### attacking use the fast_weights #########################
                optimizer.zero_grad()
            ...
            with torch.no_grad():
                ...
                losses_q[0] += loss_q  # losses_q[0] from self.net.parameters()
            ...
            if need_adv:
                ...
                label = y_qry[i]
                optimizer.zero_grad()
                adv_inp = at.attack(self.net, self.net.parameters(), data, label)  ############### use the self.net.parameters() #########################
                optimizer.zero_grad()
                with torch.no_grad():
                    logits_q_adv = self.net(adv_inp, self.net.parameters(), bn_training=True)
                    loss_q_adv = F.cross_entropy(logits_q_adv, label)
                    losses_q_adv[0] += loss_q_adv  # losses_q_adv[0] from self.net.parameters()
            ...
            with torch.no_grad():
                ...
                loss_q = F.cross_entropy(logits_q, y_qry[i])
                losses_q[1] += loss_q  # losses_q[1] from fast_weights
                # [setsz]
                pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
                correct = torch.eq(pred_q, y_qry[i]).sum().item()
                ...
                if need_adv:
                    ...
                    loss_q_adv = F.cross_entropy(logits_q_adv, label)
                    losses_q_adv[1] += loss_q_adv  # losses_q_adv[1] from fast_weights
